...
class SCR_Math3D
{
	...
	*/
	static void ScaleMatrix(out vector transform[], float scale)
	{
		if (scale == 1)
			return;
	...
	\param[out] transform Affected matrix
	*/
	static void NormalizeMatrix(out vector transform[])
	{
	...
	\param splineOrigin Origin of the spline
	\param splineEntity Entity of the spline 
	\param otherPoint Point that is being checked
	\return float Distance from spline
	*/
	static float GetDistanceFromSpline(array<vector> points, vector splineOrigin, ShapeEntity splineEntity, vector otherPoint)
	{
		float minDistance = float.MAX;
		for (int i = points.Count() - 2; i >= 0; i--)
		{
			float distance = Math3D.PointLineSegmentDistance(otherPoint, splineEntity.CoordToParent(points[i]), splineEntity.CoordToParent(points[i+1]));
