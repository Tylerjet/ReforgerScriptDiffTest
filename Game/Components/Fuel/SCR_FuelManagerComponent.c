...
class SCR_FuelManagerComponent : FuelManagerComponent
{
	...
	\param fuelTanksReceivers Optional if you want to only check specific fuel tanks. Leave null or empty to ignore
	\param ignoreIfHasFlags Ignore any fuel nodes that have the ignore flags. Leave 0 to ignore
	...
	*/
	...
	\param provider If fuel should be taken from another fuel manager
	\param fuelTanksReceivers Only transfer fuel into the given fuel managers. If empty array all are used. Note that the fuel tanks still need the CAN_RECEIVE_FUEL flag in order to receive fuel
	\param ignoreIfHasFlags Ignore any fuel nodes of provider that have the ignore flags. Leave 0 to ignore
	...
	*/
	...
	\param ignoreIfHasFlags Any nodes with this flag is skipped. Leave 0 to ignore
	...
	*/
	...
	\param fuelNodeFlags Only checks fuel nodes with given type (or types depending if all or one flag is required). Leave 0 to get all node types even non SCR_FuelNodes. Otherwise only checks SCR_FuelNodes
	\param ignoreIfHasFlags Any nodes with this flag is skipped. Leave 0 to ignore
	\param queryType If the obtained data is from fuelnodes with: The exact given type, has all given flags, has at least one given flag
	*/
	...
	/*!
	Get to currentFuel, totalfuel and fuel percentage of all nodes with the given flags
	\param fuelManagers FuelManagers
	\param[out] totalFuel Total current fuel of all fuel nodes
	\param[out] totalMaxFuel Total max fuel of all fuel nodes
	\param[out] totalFuelPercentage Total fuel percentage of all fuel nodes. Gives value from 0 to 1
	\param hasfuelNodeFlags Only checks fuel nodes with given type (or types depending if all or one flag is required). Leave 0 to get all node types even non SCR_FuelNodes. Otherwise only checks SCR_FuelNodes
	\param ignoreIfHasFlags Any nodes with this flag is skipped. Leave 0 to ignore
	\param queryType If the obtained data is from fuelnodes with: The exact given type, has all given flags, has at least one given flag
	*/
	...
	\param percentage Percentage of fuel to set
	\param fuelNodeFlag Flag of what type (or types depending if all or one flag is required) of fuel nodes can be set. Leave 0 to set all node types. Non SCR_Nodes are always valid
	\param ignoreIfHasFlags Any nodes with this flag is skipped. Leave 0 to ignore
	\param queryType If the nodes that are set have: The exact given type, has all given flags, has at least one given flag
	*/
