...
class SCR_BarrageEffectsModule : SCR_EffectsModule
{
	...
	\return Total Projectile Amount in the current barrage
	*/
	...
	*/
	...
	protected void StartBarrage()
	{
		...
		m_fCurrentProjectileDelay = Math.RandomFloatInclusive(m_fMinProjectileDelay, m_fMaxProjectileDelay);
	...
	protected void BarrageDone()
	{
		...
		GetGame().GetCallqueue().CallLater(StartBarrage, Math.RandomFloatInclusive(m_fMinBarrageDelay, m_fMaxBarrageDelay) * 1000);		
	}
};