...
class CP_SlotAIClass : CP_SlotBaseClass
{
	// prefab properties here
}

//------------------------------------------------------------------------------------------------
class CP_SlotAI : CP_SlotBase
{
	...
	void ActivateAI()
	{
		...
		{
			int iMaxUnitsInGroup = m_pAIGroup.m_aUnitPrefabSlots.Count();
			float iUnitsToSpawn = Math.Map(GetPlayersCount(), 1, GetMaxPlayersForGameMode(), Math.RandomInt(1, 3), iMaxUnitsInGroup);
			m_pAIGroup.SetMaxUnitsToSpawn(iUnitsToSpawn);
		}
		m_pAIGroup.SpawnUnits();		
	}
	
		//------------------------------------------------------------------------------------------------
	void CP_SlotAI(IEntityComponentSource src, IEntity ent, IEntity parent)
	{
		#ifdef WORKBENCH
			m_fDebugShapeColor = ARGB(100, 0x00, 0x10, 0xFF);
