...
//		SCR_GameModeSFManager pGameModeComp = SCR_GameModeSFManager.Cast(GetGame().GetGameMode().FindComponent(SCR_GameModeSFManager));
	...
	protected void SetupTask()
	{
		...
		if (!m_sTaskTitle.IsEmpty()) 
			m_pSupportEntity.SetTaskTitle(GetTask(), GetTaskTitle());
		if (!m_sTaskDescription.IsEmpty()) 
			m_pSupportEntity.SetTaskDescription(GetTask(), GetTaskDescription());
		...
		if (m_pTaskSubject)
		{
			if (!m_pTaskSubject.GetTaskTitle().IsEmpty())
				m_pSupportEntity.SetTaskTitle(GetTask(), m_pTaskSubject.GetTaskTitle());
			if (!m_pTaskSubject.GetTaskDescription().IsEmpty())
				m_pSupportEntity.SetTaskDescription(GetTask(), m_pTaskSubject.GetTaskDescription());
		}
		else
		{
		...
		PrintFormat("CP: ->Task: Generating task %1. Description: %2", string.Format(WidgetManager.Translate(m_pTask.GetTitle())), string.Format(WidgetManager.Translate(m_pTask.GetDescription())));
	}
