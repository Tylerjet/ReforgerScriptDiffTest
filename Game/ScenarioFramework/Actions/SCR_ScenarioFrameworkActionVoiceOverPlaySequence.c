...
class SCR_ScenarioFrameworkActionVoiceOverPlaySequence : SCR_ScenarioFrameworkActionBase
{
	...
	[Attribute(desc: "Name of the sequence as defined in Voice Over Data Config")]
	...
	[Attribute(defvalue: "1", desc: "If Voice Sequence should play right now or wait for the current voice sequence to finish and then play it")]
	...
	protected ref map<SCR_EVoiceoverActor, IEntity> m_mActorEntities = new map<SCR_EVoiceoverActor, IEntity>();
	...
	override void OnActivate(IEntity object)
	{
		...
		if (m_ActorGetter)
		{
			...
			if (entityArrayWrapper)
			{
				actorEntities = entityArrayWrapper.GetValue();
				if (!actorEntities || actorEntities.IsEmpty())
					return;
			}
			else
			{
				...
				actorEntities.Insert(playerEntity);
			}
		}	
