...
{	
	...
	protected static ref ScriptCallQueue m_CallQueue = new ScriptCallQueue();
	...
	void PostInit()
	{
		...
		scriptedGameMode.GetOnPlayerConnected().Insert(SyncDebugShapes);
	}
	...
	void CheckLayerTasksAfterInit(SCR_ScenarioFrameworkLayerBase layer)
	{
		...
		if (m_iCurrentlySpawnedLayerTasks == m_aLayerTasksToBeInitialized.Count())
			...
			SCR_ScenarioFrameworkSystem.GetCallQueue().CallLater(AfterLayerTasksInit, 1000);
	}
	//---- REFACTOR NOTE END ----
	...
	void GenerateSingleTask(int index)
	{
		...
		// This can be probably removed - punish me on code review if I forgot about it
		//m_iMaxNumberOfTasks--;
	}
	...
	void PrepareDynamicDespawn()
	{
		...
		SCR_ScenarioFrameworkSystem.GetCallQueue().CallLater(CheckDistance, 1000 * m_iUpdateRate, true);
	}
