...
class SCR_CareerSpecializationsUI: ScriptedWidgetComponent
{
	...
	protected ResourceName m_SpProgressLayout;
	...
	void FillSpecializations(SCR_PlayerData playerData, ResourceName statsLayout, ResourceName headerStatsLayout, ResourceName progressionStatsLayout)
	{
		...
		m_StatsLayout = statsLayout;
		m_HeaderStatsLayout = headerStatsLayout;
		m_ProgressionStatsLayout = progressionStatsLayout;
	...
	protected void SetUpSpecializationDisplay()
	{
		...
		for (i = 0; i < maxCount; i++)
		{
			...
			if (!m_bShowProgression)
				tempW = SCR_CareerUI.CreateStatEntry(specializationsPanelWidget, m_StatsLayout);
			else
				tempW = SCR_CareerUI.CreateProgressionStatEntry(specializationsPanelWidget, m_ProgressionStatsLayout);
	...
	protected Widget CreateSpecializationProgressButtonWidget(Widget container, int id)
	{
		...
		Widget SpecializationProgressButton = Widget.Cast(workspace.CreateWidgets(m_SpProgressLayout, container));
	...
	void FillWarCrimes()
	{
		...
		if (m_PlayerData.GetWarCrimes() > 0)
		{
			if (m_PlayerData.GetWarCrimesHarmingFriendlies() > 0)
			{
