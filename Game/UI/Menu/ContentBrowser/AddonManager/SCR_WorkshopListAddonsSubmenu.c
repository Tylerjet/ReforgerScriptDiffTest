class SCR_WorkshopListAddonsSubmenu : SCR_SubMenuBase
{
	...
	protected SCR_NavigationButtonComponent m_NavDeleteAll;
	protected SCR_NavigationButtonComponent m_NavOpenDetails;
	protected SCR_NavigationButtonComponent m_NavUpdate;
	
	protected SCR_NavigationButtonComponent m_NavOpenTools;
	protected SCR_NavigationButtonComponent m_NavOpenPresets;
	...
	protected ref ScriptInvoker<> Event_OnListCreated = new ScriptInvoker();
	...
	protected void FocusFirstLine()
	{
		...
		if (m_Widgets.m_EnabledAddonsPanel && m_Widgets.m_EnabledAddonsPanel.GetChildren())
			workspace.SetFocusedWidget(m_Widgets.m_EnabledAddonsList.GetChildren());
		else if (m_Widgets.m_DisabledAddonsPanel && m_Widgets.m_DisabledAddonsPanel.GetChildren())
			workspace.SetFocusedWidget(m_Widgets.m_DisabledAddonsPanel.GetChildren());
		
		if (!m_aEntriesDisabled.IsEmpty())
			workspace.SetFocusedWidget(m_aEntriesDisabled[0]);
		else if (!m_aEntriesEnabled.IsEmpty())
			workspace.SetFocusedWidget(m_aEntriesEnabled[0]);
	}
	...
	protected void CreateNavigationButtons()
	{
		...
		//m_NavOpenPresets = CreateNavigationButton("WorkshopPrimary", "#AR-Workshop_AddonsPresetTitle", false);
		//m_NavOpenTools = CreateNavigationButton("MenuNavRight_NoScroll", "#AR-Workshop_AddonsExportDialogTitle", false);
		
		//m_NavDeleteAll = CreateNavigationButton("MenuUnsubscribeAll", "#AR-Workshop_ButtonDelete", false);
		m_NavEnableAll = CreateNavigationButton("MenuFavourite", "#AR-Workshop_TabName_All", false);
	...
	protected void FocusLastWidget()
	{
		...
		{
			auto comp = SCR_WorkshopAddonLineComponent.Cast(list[i].FindHandler(SCR_WorkshopAddonLineComponent));
			if (!comp)
				continue;

			if (comp.GetWorkshopItem() == m_LastEnabledItem)
			{
				//GetGame().GetCallqueue().CallLater(ApplyLineFocus, 1000, false, list[i], comp);
				//GetGame().GetWorkspace().SetFocusedWidget(list[i]);
	...
	protected void OnNavDeleteActivated()
	{
		...
		//m_FocusedAddonLine.OnDeleteButton();	
	}
	...
	protected void OnLineMouseLeave(SCR_WorkshopAddonLineComponent comp)
	{
		...
		auto item = m_FocusedAddonLine.GetWorkshopItem();
		m_Widgets.m_AddonInfoPanelComponent.SetWorkshopItem(item);
	}
