...
{	
	...
	const string VON_ADDITIONAL_TRANSMISSIONS_PATH = "{284AD36540589982}UI/layouts/HUD/VON/VoNAdditionalRecieves.layout";
	...
	protected void OnSlotUIResize()
	{
		...
		m_iTransmissionSlots = ((int)height / HEIGHT_DEVIDER) - 1;
	...
	protected void UpdateWidgets()
	{
		...
		else
		{
			...
			{
				Widget w = GetGame().GetWorkspace().CreateWidgets(VON_RECEIVING_TRANSMISSION_PATH, m_wVerticalLayout);
				if (w)
				{
					w.SetVisible(false);
