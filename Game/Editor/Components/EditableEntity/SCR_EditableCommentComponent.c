...
{	
	...
	*/
	...
	override void _WB_AfterWorldUpdate(IEntity owner, float timeSlice)
	{
		...
		if (!api.IsEntityVisible(genericOwner) || !api.IsEntityLayerVisible(src.GetSubScene(), src.GetLayerID())) return;
		...
		else
		{
			...
			if (vector.DistanceSq(pos, cameraTransform[3]) > _WB_DRAW_DISTANCE) return;
		}
