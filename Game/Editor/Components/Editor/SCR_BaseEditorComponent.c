...
class SCR_BaseEditorComponentClass: ScriptComponentClass
{
	...
	\param type Type of requested component (not its prefab data, i.e., without *Class suffix!)
...
{	
	...
	*/
	...
	\param attempt Number of previous attempts to activate the component.
	\return True if the component is activated, false if it isn't (another attempt will follow in the next frame).
	*/
	protected bool EOnEditorActivateAsync(int attempt) { return true; }
	...
	*/
	...
	\param attempt Number of previous attempts to deactivate the component.
	\return True if the component is activated, false if it isn't (another attempt will follow in the next frame).
	*/
	protected bool EOnEditorDeactivateAsync(int attempt) { return true; }
	...
	sealed void OnPostActivateBase()
	{
		...
		if (prefabData) SCR_BaseEditorEffect.Activate(prefabData.GetEffectsActivate(), this);
	}
	sealed void OnDeactivateBase()
	{
		...
		SCR_BaseEditorComponentClass prefabData = SCR_BaseEditorComponentClass.Cast(GetEditorComponentData());
		if (prefabData) SCR_BaseEditorEffect.Activate(prefabData.GetEffectsDeactivate(), this);
	}
	...
	*/
	static int GetAllInstances(typename type, out notnull array<Managed> outComponents)
	{
		...
		SCR_EditorManagerEntity editorManager = SCR_EditorManagerEntity.GetInstance();
		if (!editorManager) return 0;
		
		Managed component = editorManager.FindComponent(type);
		if (component) outComponents.Insert(component);
	...
	*/
	...
	void InitOwner()
	{
		...
		if (m_Owner.GetOnDebug()) m_Owner.GetOnDebug().Insert(EOnEditorDebug);
		
		m_Manager = SCR_EditorManagerEntity.Cast(m_Owner);
		if (!m_Manager) m_Manager = m_Owner.GetManager();		
	...
	{	
		if (SCR_Global.IsEditMode(ent)) return; //--- Run-time only
