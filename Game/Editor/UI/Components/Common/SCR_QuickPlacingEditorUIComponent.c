...
{
	const float ANIMATION_THRESHOLD = 0.01;
	...
	protected void SetCoef(float coef, bool instant = false)
	{
		...
		else
		{
			...
			if (menu) menu.GetOnMenuUpdate().Insert(OnMenuUpdate);
		}
	}
	protected bool SetOffsets(float progress)
	{
		float offsetLeft, offsetTop, offsetRight, offsetBottom;
		FrameSlot.GetOffsets(m_ExpandWidget, offsetLeft, offsetTop, offsetRight, offsetBottom);
		...
		FrameSlot.SetOffsets(m_ExpandWidget, offsetLeft, offsetTop, offsetRight, offsetBottom);
	...
	{
		if (w.GetName() == m_sActiveWidgetName && m_ExpandWidget)
		{
			Show(true);
		}
		return false;
	...
	{
		m_ExpandWidget = w.FindAnyWidget(m_sExpandWidgetName);
		if (!m_ExpandWidget) return;
		
		float offsetLeft, offsetTop, offsetRight, offsetBottom;
		FrameSlot.GetOffsets(m_ExpandWidget, offsetLeft, m_fDefaultOffset, offsetRight, offsetBottom);
		
		SetCoef(m_fHideCoef, true);

		ScriptInvoker onToggle = ButtonActionComponent.GetOnAction(w);
		if (onToggle) onToggle.Insert(OnShortcut);
	}
