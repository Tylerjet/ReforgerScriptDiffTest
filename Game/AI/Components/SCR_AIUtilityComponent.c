...
class SCR_AIUtilityComponent : SCR_AIBaseUtilityComponent
{
	...
	//------------------------------------------------------------------------------------------------
	SCR_AIBehaviorBase EvaluateBehavior(BaseTarget unknownTarget, SCR_AIMessageBase commandMessage, SCR_AIMessageBase infoMessage)
	{
	...
	bool CanIndependentlyMove()
	{
		...
		foreach (SCR_AIActionBase action : m_aActions)
		{
			if (action.Type() == SCR_AICombatMoveGroupBehavior)
				return false;
		}
		return true;
	}
