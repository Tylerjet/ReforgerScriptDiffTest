...
class SCR_DataCollectorBasicActionsModule : SCR_DataCollectorModule
{
	...
	protected override void AddInvokers(IEntity player)
	{
		...
		SCR_CompartmentAccessComponent compartmentAccessComponent = SCR_CompartmentAccessComponent.Cast(SCR_ChimeraCharacter.Cast(player).GetCompartmentAccessComponent());
	...
	override void Execute(IEntity owner, float timeTick)
	{
		...
		for (int i = m_mTrackedPlayers.Count() - 1; i >= 0; i--)
		{
			...
			{
				StatsVisualization.Get(EBasicActionsModuleStats.SessionDuration).SetText(playerData.GetSessionDuration().ToString());
			}
			...
			if (StatsVisualization)
			{
				StatsVisualization.Get(EBasicActionsModuleStats.DistanceWalked).SetText(playerData.GetDistanceWalked().ToString());
			}
