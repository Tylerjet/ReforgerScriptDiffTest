...
class SCR_DataCollectorDriverModule : SCR_DataCollectorModule
{
	...
	override void Update(IEntity owner, float timeTick)
	{
		...
		for (int i = m_mTrackedPlayersInVehicles.Count() - 1; i >= 0; i--)
		{
			...
			{
				m_StatsVisualization.Get(EDriverModuleStats.MetersDriven).SetText(playerData.GetDistanceDriven().ToString());
				m_StatsVisualization.Get(EDriverModuleStats.MetersAsOccupant).SetText(playerData.GetDistanceAsOccupant().ToString());
				m_StatsVisualization.Get(EDriverModuleStats.PointsAsDriver).SetText(playerData.GetPointsAsDriverOfPlayers().ToString());
				m_StatsVisualization.Get(EDriverModuleStats.RoadKills).SetText(playerData.GetRoadKills().ToString());
				m_StatsVisualization.Get(EDriverModuleStats.AIRoadKills).SetText(playerData.GetAIRoadKills().ToString());
				m_StatsVisualization.Get(EDriverModuleStats.FriendlyRoadKills).SetText(playerData.GetFriendlyRoadKills().ToString());
				m_StatsVisualization.Get(EDriverModuleStats.FriendlyAIRoadKills).SetText(playerData.GetFriendlyAIRoadKills().ToString());
				m_StatsVisualization.Get(EDriverModuleStats.PlayersDiedInVehicle).SetText(playerData.GetPlayersDiedInVehicle().ToString());
			}				
		}
