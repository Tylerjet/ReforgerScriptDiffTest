class SCR_ResourceContainerVehicleUnloadAction : SCR_ScriptedUserAction
{
	...
	override void PerformAction(IEntity pOwnerEntity, IEntity pUserEntity) 
 	{
		...
		||	!m_ResourceGenerator	&& !m_ResourceComponent.GetGenerator(EResourceGeneratorID.VEHICLE_UNLOAD, m_eResourceType, m_ResourceGenerator)
	...
	{	
		...
		outName = WidgetManager.Translate(GetUIInfo().GetDescription(), m_fCurrentTransferValue, m_fCurrentResource, m_ResourceConsumer.GetAggregatedMaxResourceValue());
		outName = WidgetManager.Translate("#AR-ActionFormat_SupplyCost", outName, m_ResourceGenerator.GetAggregatedResourceValue(), m_ResourceGenerator.GetAggregatedMaxResourceValue());
	...
	override event bool CanBePerformedScript(IEntity user)
	{
		...
		if (m_ResourceGenerator.GetAggregatedMaxResourceValue() == 0.0)
			SetCannotPerformReason("#AR-Supplies_CannotPerform_Vehicle_NoStorage");
		else if (m_fCurrentResource == 0.0)
			SetCannotPerformReason("#AR-Supplies_CannotPerform_Vehicle_StorageEmpty");
		else if (m_fCurrentTransferValue == 0.0)
	...
	override event void Init(IEntity pOwnerEntity, GenericComponent pManagerComponent)
	{
		...
		m_ResourceGenerator	= m_ResourceComponent.GetGenerator(EResourceGeneratorID.VEHICLE_UNLOAD, m_eResourceType);
		m_ResourceConsumer	= m_ResourceComponent.GetConsumer(EResourceGeneratorID.VEHICLE_UNLOAD, m_eResourceType);
	}
}