...
class SCR_GarbageManager : GarbageManager
{
	...
	{
		SCR_BaseInventoryItemComponent itemComponent = SCR_BaseInventoryItemComponent.Cast(ent.FindComponent(SCR_BaseInventoryItemComponent));
		if (itemComponent)
		{
			if (!itemComponent.CanBeInserted())
				return false;
		}
