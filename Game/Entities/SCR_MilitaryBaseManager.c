...
class SCR_MilitaryBaseManager : GenericEntity
{
	...
	{
		IEntity ent = editComponent.GetOwner();
		IEntity sibling;
		...
		while (ent)
		{
			// Process transform callback on child entities without SCR_EditableEntityComponent
			// If SCR_EditableEntityComponent is present on owner, skip it as the callback is executed automatically
			if (!checkeEditableEntityComponent || !SCR_EditableEntityComponent.Cast(ent.FindComponent(SCR_EditableEntityComponent)))
			{
				baseComponent = SCR_MilitaryBaseComponent.Cast(ent.FindComponent(SCR_MilitaryBaseComponent));
				logicComponent = SCR_MilitaryBaseLogicComponent.Cast(ent.FindComponent(SCR_MilitaryBaseLogicComponent));
			...
			checkeEditableEntityComponent = true;
		}
	}
