...
{
	[Attribute("New comment", UIWidgets.EditBox, "Write any comment you wish")]
	string m_Comment;
	...
	private void DrawComment()
	{
		...
		#ifdef WORKBENCH
			m_WB_Width = m_Comment.Length() * m_Size * distScale * textWidthScale;
			m_WB_Height = m_Size * distScale;
		#endif
		...
		override void _WB_GetBoundBox(inout vector min, inout vector max, IEntitySource src)
		{
			if (m_FaceCamera)
			{
				min = Vector(m_WB_Width * -0.5, m_WB_Height * -0.5, m_WB_Width * -0.5);
				max = Vector(m_WB_Width * 0.5, m_WB_Height * 0.5, m_WB_Width * 0.5);
			}
			else
			{
				min = Vector(m_WB_Width * -0.5, m_WB_Height * -0.5, m_WB_Height * 0.25 * -0.5);
				max = Vector(m_WB_Width * 0.5, m_WB_Height * 0.5, m_WB_Height * 0.25 * 0.5);
			}
		}
