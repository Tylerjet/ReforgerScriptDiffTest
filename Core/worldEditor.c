class WorldEditorAPI
{
	...
	proto native bool BeginEditSequence(IEntitySource entSrc);	//!< Together with EndEditSequence it can be used to define a block where entity will never be re-initilized. Usefull for multiple edit actions upon single entity
	proto native bool EndEditSequence(IEntitySource entSrc);	//!< Together with BeginEditSequence it can be used to define a block where entity will never be re-initilized. Usefull for multiple edit actions upon single entity
	proto native bool IsEditSequenceActive(IEntitySource entSrc);	//!< Whether it's time between BeginEditSequence and EndEditSequence
	...
	proto native bool IsCreatingEntityInWindow(); //!< true if a user created an entity in editor window - e.g. drag and drop from hierarchy etc.
	...
	//! Returns the number of entitis in the editor
	proto native int GetEditorEntityCount();

	//! Returns an entity in the editor on given index. For count \see GetEditorEntityCount.
	proto native IEntitySource GetEditorEntity(int index);

	/**
	\brief Shows a selection dialog with provided items where user can select one or multiple items
	...
	*/
	...
	/**
	\brief Modifies the terrain height at specified position based on given brush
	 \param xWorldPos X coordinate of the modification position.
	 \param yWorldPos Y coordinate of the modification position.
	 \param operation Operation to perform on the terrain.
	 \param toolDesc Parameters of the operaion.
	 \param shape Shape of the brush.
	 \param interpFunc Interpolation function used when creating the brush.
	*/
	proto native void ModifyHeightMap(float xWorldPos, float zWorldPos, FilterMorphOperation operation, TerrainToolDesc toolDesc, FilterMorphShape shape, FilterMorphLerpFunc interpFunc);
	/**
	\brief Modifies the terrain height at specified position based on given map
	 \param xWorldPos X coordinate of the modification position.
	 \param yWorldPos Y coordinate of the modification position.
	 \param operation Operation to perform on the terrain.
	 \param toolDesc Parameters of the operaion.
	...
	*/
	proto native void ModifyHeightMapUserShape(float xWorldPos, float zWorldPos, FilterMorphOperation operation, TerrainToolDesc toolDesc, array<float> userShape, UserShapeFilter userShapeFilter);
	/**
	\brief Modifies the terrain layers at specified position based on given brush
	 \param xWorldPos X coordinate of the modification position.
	 \param yWorldPos Y coordinate of the modification position.
	 \param operation Operation to perform on the terrain.
	 \param toolDesc Parameters of the operaion.
	 ...
	 \param interpFunc Interpolation function used when creating the brush.
	*/
	proto native void ModifyLayers(float xWorldPos, float zWorldPos, FilterMorphOperation operation, TerrainToolDesc toolDesc, FilterMorphShape shape, FilterMorphLerpFunc interpFunc);
	/**
	\brief Modifies the terrain layers at specified position based on given map
	 \param xWorldPos X coordinate of the modification position.
	 \param yWorldPos Y coordinate of the modification position.
	 \param operation Operation to perform on the terrain.
	 \param toolDesc Parameters of the operaion.
	 ...
	 \param userShapeFilter Filtering to use when the tool size is not the same as the userShape array dimension.
	*/
	proto native void ModifyLayersUserShape(float xWorldPos, float zWorldPos, FilterMorphOperation operation, TerrainToolDesc toolDesc, array<float> userShape, UserShapeFilter userShapeFilter);
	...
	/**
	\brief Returns horizontal mouse cursor position in active scene window (usually it's perspective window but it can be also one of Top, Back, Right view)
	...
	*/
	...
	/**
	\brief Returns vertical mouse cursor position in active scene window (usually it's perspective window but it can be also one of Top, Back, Right view)
	...
	*/
	...
	/*!
	Set value of potentially nested variable. The whole path must exist, this function is just to set the value.
	To create \see CreateObjectArrayVariableMember
	Traverses containers from `topLevel` using `containerIdPath`. For each entry selects property with given index.
	...
	\param variable The actual variable to set
