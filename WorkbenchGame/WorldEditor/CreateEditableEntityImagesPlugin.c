...
class CreateEditableEntityImagesPlugin: PrefabEditingPluginBase
{
	...
	protected bool ValidateScreen(WorldEditorAPI api)
	{
		...
		if (screenW == m_iWidth && screenH == m_iHeight) return true;
	...
	protected bool ValidatePrefabs(out notnull map<ResourceName, string> outPrefabs)
	{
		...
		if (!GetPrefabs(selectedPrefabs)) return false;
		...
		foreach (ResourceName prefab: selectedPrefabs)
		{
			...
			SCR_EditableEntityUIInfo info = SCR_EditableEntityUIInfo.Cast(SCR_EditableEntityComponentClass.GetInfo(componentSource));
			if (!info)
			{
		...
		if (!outPrefabs.IsEmpty()) return true;
	...
	protected void CaptureImages(WorldEditorAPI api, map<ResourceName, string> prefabs)
	{
		...
		if (player) player.ClearFlags(EntityFlags.VISIBLE, true);
		...
		{
			if (!api.IsGameMode()) return; //--- Terminated by user
			...
			string addon = SCR_AddonTool.GetResourceLastAddon(prefab);
	...
	protected void GetEntities(BaseWorld world, out notnull array<IEntity> outEntities, array<IEntity> baseEntities = null)
	{
		...
		if (baseEntities)
		{
			...
			{
				if (baseEntities.Find(entity) == -1) outEntities.Insert(entity);
			}
		}
		else
		{
